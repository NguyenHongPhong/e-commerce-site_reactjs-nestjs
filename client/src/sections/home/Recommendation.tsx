            ...
            <div className="grid grid-cols-5 mt-7 gap-5">
                {products && products.map((product: any, i: number) => (
                    <Link key={i} to={""} className="h-52 w-44 hover:shadow-2xl relative group hover:border-blue-600">
                        <div className=" bg-cover w-full h-3/5 absolute
                        bg-center group-hover:scale-95"
                            style={{
                                backgroundImage: `url(${product.product_Images?.[1]?.url ? product.product_Images?.[1]?.url :
                        ...
                        <div className="absolute bottom-1 px-1.5">
                            <span className="text-[12px] line-clamp-2 group-hover:scale-95">{product.title}</span>
                        </div>
